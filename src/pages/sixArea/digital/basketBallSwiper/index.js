...
class AdsSwiper extends Component {
    ...
    render() {
        let { newSwiperItems, countheight, activeIndex } = this.state
        return (
            <SwiperContainer ref="swiperContainer">
                <div className="swiper-content" ref="swiperContent" style={{ height: countheight }}>
                    {
    ...
    handleInit() {
        //一个图片的宽度
        this.swiperIw = this.refs.swiperContainer.offsetHeight;
        console.log(this.refs.swiperContainer.offsetHeight)
        //swiperContent的宽度
        let height = this.swiperIw * this.state.newSwiperItems.length;
    ...
    autoplay() {
        this.timer = setInterval(() => {
            if (this.state.swiperIndex == this.state.len) {
                ...
                this.refs.swiperContent.style.top = 0;
            } else {
                this.setState({
    ...
    //轮播的原理
    toImg() {
        this.refs.swiperContent.style.transition = 'top 0.2s ease-in-out';
        this.refs.swiperContent.style.top = -(this.state.swiperIndex * this.swiperIw) + 'px';
    ...
    handleTouchMove() {
        this.refs.swiperContent.addEventListener("touchmove", (e) => {
            ...
            if (this.distanceX > 0 && this.state.swiperIndex == 0) {
                this.refs.swiperContent.style.top = -(this.swiperIw * this.state.len) + this.distanceX + 'px';
            } else if (this.distanceX < 0 && this.state.swiperIndex == this.state.len) {
                this.refs.swiperContent.style.top = this.distanceX + 'px';
            } else {
                this.refs.swiperContent.style.top = -(this.state.swiperIndex * this.swiperIw) + this.distanceX + 'px';
            }
        })
    }
    handleTouchEnd(){
            ...
            if(Math.abs(this.distanceX) > this.swiperIw/5){
                ...
                if(this.distanceX>0){
                    if(this.state.swiperIndex == 0){
                        ...
                        this.refs.swiperContent.style.top = -(this.state.len * this.swiperIw)+'px';
                    }else{
                        this.setState({
                            swiperIndex:--this.state.swiperIndex
                        })
                ...
                if(this.state.swiperIndex == 0){
                    ...
                    this.refs.swiperContent.style.top = -(this.swiperIw * this.state.len)+'px';
                }else{
                    this.refs.swiperContent.style.top = -(this.state.swiperIndex * this.swiperIw)+'px';
                }
            }
