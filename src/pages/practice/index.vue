<template>
  <div class="app-container mainContentContainer" v-if="questionListLen">
      ...
      <swiper-item v-for="(item,index) in questionList" :key="index" style="overflow-y: scroll;">
        <template v-if="index - curQuestionId < 2 && index - curQuestionId > -2">
          <div v-if="qType!='1'&& qType!='2'" class="q-content">
            ...
            <div v-if="item.type<4 && activeMode == 1" class="radio-box">
              <ai-radio
                v-if="item.type!=='2'"
            ...
            <div v-if="item.type>3" class="no-answer">
              <div v-show="activeMode===0">此题型不支持在线作答，交卷后查看答案</div>
            </div>
            <template v-if="qType!='1' && qType!='2' && activeMode == 0">
          ...
          <div
            class="section-title"
            v-if="(qType=='2')|| (qType=='1')"
            @tap.stop="handleClickAnalysis"
          >
            <div class="section-name">题目解析：</div>
            <div class="icon">
              <van-icon name="arrow" v-if="!checkAnalysis" />
              <van-icon name="arrow-down" v-else />
            </div>
          </div>
          <template v-if="(qType=='2')|| (qType=='1')">
            <div class="analysis-container" v-show="checkAnalysis">
    ...
    <div class="footer">
      ...
      <div class="footer-box" v-else>
        ...
        <div class="time-box">
          ...
          >
            <template slot="icon">
              <div class="icon-timer" slot="icon">
...
export default {
    ...
    curQuestionId (newValue) {
      this.$nextTick(() => {
        if (newValue < this.questionListLen) {
          if (newValue > this.questionListLen - 6 && this.questionListLen < this.questionTotal) {
            if (!this.isLoading) {
              this.getQuestions()
            }
            this.curQuestionId = newValue
          }
        } else {
  ...
  computed: {
    ...
    qType () {
      return this.$store.getters.questionType.toString()
    }
  },
  ...
  onShow () {
    ...
    this.mainH = this.$store.getters.mainWrapHeight - 50 - 44 + 'px'
  ...
  methods: {
    _init () {
      ...
      this.queryQuestionParam.pageNum = 0
      // mpvue.setStorageSync('firstPractice', 1)
      this.isFirst = this.checkIsFirst()
      this.$store.dispatch('updateStartTime', currentDateTime())
    },
    getQuestions () {
      if (this.queryQuestionParam.pageNum !== 'undefined') {
        this.queryQuestionParam.pageNum++
      }
      this.isLoading = true
    ...
    handleClickAnalysis () {
      this.checkAnalysis = !this.checkAnalysis
    },
    handleClickTo (id) {
      mpvue.navigateTo({
        url: '../feedback_corrective/main?qid=' + id + '&curIndex=' + this.curQuestionId + 1
      })
    },
    updateCurQuestionId () {
    ...
    handleSubmitApi (data) {
      this.timerSwitch && (this.timerSwitch = false)
    ...
    goBack () {
      if (['1', '2'].indexOf(this.$store.getters.questionType.toString()) > -1 || this.questionList.length < 1) {
